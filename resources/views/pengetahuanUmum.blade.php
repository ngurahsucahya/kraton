...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    <head>
        ...
        <title>Kraton - Pengetahuan Umum</title>
                ...
                <div class="mt-8 bg-white dark:bg-gray-800 overflow-hidden shadow sm:rounded-lg">
                    ...
                    <div style="padding: 30px; width: 600px; word-wrap: normal;">
                        @if(request()->session()->has('time') && request()->session()->has('pertanyaan'))
                            ...
                            @if(isset($_GET["page"]))
                                @if($_GET["page"] >= 1 && $_GET["page"] <= request()->session()->get('pertanyaan'))


                            @foreach($pertanyaan as $pertanyaan)
                                <br>
                                <span class="badge badge-danger" style="color: white; font-size: 140%;">pertanyaan {{ isset($_GET["page"]) ? $_GET["page"] : "1" }}</span>
                                 <div class="badge badge-dark" id="countdown" style="color: white; font-size: 140%;">Mulai hitung mundur</div><br><br>
    ...
    border-radius: 0.25rem;">{{$pertanyaan->pertanyaan}}</div><br><br>
                                <label class="container" style="border-width: 3px;border-style: solid;border-image: linear-gradient(to right, #ac4eff, #44d2f4) 1;">{{$pertanyaan->A}}
                                    <input type="radio" name="pertanyaan{{$pertanyaan->id}}" id="pertanyaan{{$pertanyaan->id}}A"  value="A">
                                    <span class="checkmark"></span>
                                </label>
                                <label class="container" style="border-width: 3px;border-style: solid;border-image: linear-gradient(to right, #ac4eff, #44d2f4) 1;">{{$pertanyaan->B}}
                                    <input type="radio" name="pertanyaan{{$pertanyaan->id}}" id="pertanyaan{{$pertanyaan->id}}B"  value="B">
                                    <span class="checkmark"></span>
                                </label>
                                <label class="container" style="border-width: 3px;border-style: solid;border-image: linear-gradient(to right, #ac4eff, #44d2f4) 1;">{{$pertanyaan->C}}
                                    <input type="radio" name="pertanyaan{{$pertanyaan->id}}" id="pertanyaan{{$pertanyaan->id}}C"  value="C">
                                    <span class="checkmark"></span>
                                </label>
                                <label class="container" style="border-width: 3px;border-style: solid;border-image: linear-gradient(to right, #ac4eff, #44d2f4) 1;">{{$pertanyaan->D}}
                                    <input type="radio" name="pertanyaan{{$pertanyaan->id}}" id="pertanyaan{{$pertanyaan->id}}D"  value="D">
                                    <span class="checkmark"></span>
                                </label>
                                <center>
                                    @if(!isset($_GET["page"]))
                            ...
                            <form name="check-first" id="check-first" method="post" action="{{url('check-first')}}">
                                <center>
                                    @csrf
                                    <label for="time"><b>Durasi menjawab setiap pertanyaan (15-120 detik.)</b></label>
                                    ...
                                    <br>
                                    <button type="submit" class="btn btn-primary">AYO MULAI!</button>
                                </center>
                            </form>
                            @endif
                ...
                @if(request()->session()->has('time') && request()->session()->has('pertanyaan'))
                    <center><br>
                    <span class="badge badge-info" style="color: white; font-size: 130%;">{{request()->session()->get('time')}} detik setiap pertanyaan</span>
                ...
                <script>
                    function myFunc() {
                        ...
                        if (confirm("Semua pekerjaan kamu akan hilang. Apakah kamu yakin?") == true) {
        ...
        @if(isset($_GET["page"]) && $_GET["page"] >= 1 && $_GET["page"] <= request()->session()->get('pertanyaan'))
        <script>
            var sH = sessionStorage.getItem({{$pertanyaan->id}});
            var timeleft = {{request()->session()->get('time')}};
            if(sessionStorage.getItem({{$pertanyaan->id}} + "time") === null){
            sessionStorage.setItem({{$pertanyaan->id}} + "time", timeleft);
            }

            if(sessionStorage.getItem({{$pertanyaan->id}} + "out") != "out"){
                var downloadTimer = setInterval(function(){
                    if(sessionStorage.getItem({{$pertanyaan->id}} + "time") <= 0){
                        ...
                        document.getElementById("pertanyaan" + {{$pertanyaan->id}} + "B").disabled=true;
                        document.getElementById("pertanyaan" + {{$pertanyaan->id}} + "C").disabled=true;
                        document.getElementById("pertanyaan" + {{$pertanyaan->id}} + "D").disabled=true;
            ...
            document.addEventListener("DOMContentLoaded", function(event) {
                if (sessionStorage.getItem({{$pertanyaan->id}}) !== null) {
                    if(sH == "A" || sH == "B" || sH == "C" || sH == "D"){
                    document.getElementById("pertanyaan" + {{$pertanyaan->id}} + sessionStorage.getItem({{$pertanyaan->id}})).checked = true;
                    }
                    document.getElementById("pertanyaan" + {{$pertanyaan->id}} + "A").disabled=true;
                    document.getElementById("pertanyaan" + {{$pertanyaan->id}} + "B").disabled=true;
                    document.getElementById("pertanyaan" + {{$pertanyaan->id}} + "C").disabled=true;
                ...
                const radioButtons = document.querySelectorAll('input[name="pertanyaan{{$pertanyaan->id}}"]');
                btn.addEventListener("click", () => {
                    ...
                    if(sessionStorage.getItem({{$pertanyaan->id}}) === null){
                        sessionStorage.setItem({{$pertanyaan->id}}, selectedSize);
                    }else if(sessionStorage.getItem({{$pertanyaan->id}}) !== selectedSize)
                    {
                        var sH = sessionStorage.getItem({{$pertanyaan->id}});
                        if(sH == "A" || sH == "B" || sH == "C" || sH == "D"){
                            alert("The answer you chose for this pertanyaan was "+ sessionStorage.getItem({{$pertanyaan->id}}) +". It cannot be changed after the time has expired or once answer is selected.")
                            }else{
                            alert("Waktu sudah habis. Kamu tidak bisa menjawab pertanyaan ini.")
                        }
                    }
                });
